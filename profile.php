...
if (!empty($_POST["action"])) {
	...
	if ($action == "changepwd") {
		...
		try {
			$stmt = $smarty->dbh()->prepare("UPDATE {$opt["table_prefix"]}users SET fullname = ?, email = ? WHERE userid = ?");
...
try {
	$stmt = $smarty->dbh()->prepare("SELECT fullname, email FROM {$opt["table_prefix"]}users WHERE userid = ?");
