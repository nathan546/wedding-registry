...
if ($action == "insert" || $action == "update") {
	...
	if ($RSVP == "") {
		$haserror = true;
		$RSVP_error = "RSVP count is required.";
	}
}
...
if ($action == "delete") {
	...
	$stmt = $smarty->dbh()->prepare("SELECT username, fullname, email, RSVP, email_msgs, approved, admin FROM {$opt["table_prefix"]}users WHERE userid = ?");
	...
	if (!$haserror) {
		...
		$stmt = $smarty->dbh()->prepare("INSERT INTO {$opt["table_prefix"]}users(username,password,fullname,email,RSVP,email_msgs,approved,admin) VALUES(?, {$opt["password_hasher"]}(?), ?, ?, ?, ?, ?, ?)");
		$stmt->bindParam(1, $username, PDO::PARAM_STR);
		$stmt->bindParam(2, $pwd, PDO::PARAM_STR);
		$stmt->bindParam(3, $fullname, PDO::PARAM_STR);
		$stmt->bindParam(4, $email, PDO::PARAM_STR);
		$stmt->bindParam(5, $RSVP, PDO::PARAM_STR);
		$stmt->bindParam(6, $email_msgs, PDO::PARAM_BOOL);
	...
	if (!$haserror) {
		...
		$stmt->bindParam(4, $RSVP, PDO::PARAM_STR);
...
$stmt = $smarty->dbh()->prepare("SELECT userid, username, fullname, email, RSVP, email_msgs, approved, admin FROM {$opt["table_prefix"]}users ORDER BY username");
$stmt->execute();
$users = array();
...
if (($_SESSION["admin"] == 1) && $opt["newuser_requires_approval"]) {
	$query = "SELECT userid, fullname, email, approved " .
...
if (isset($RSVP_error)) {
	$smarty->assign('RSVP_error', $RSVP_error);
}
